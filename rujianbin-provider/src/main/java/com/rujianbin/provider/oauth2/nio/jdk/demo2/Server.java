package com.rujianbin.provider.oauth2.nio.jdk.demo2;
 ...
 * 以下的服务端一个线程监听两个服务，整个服务端只有一个阻塞的方法：

 //选择一组键，其相应的通道已为 I/O 操作准备就绪。

 this.selector.select();

 当客户请求服务器的时候，那么这造成了TCP没有面向连接的假象，其实至少在传输数据的时候是连接的，只是在一次I/O请求结束之后服务器端就
 把连接给断开，继而继续去处理更多的请求。而在客户端，可以看到也是遇到一次请求的时候就connect服务端一次。所以TCP还是面向连接的。
 */
public class Server implements Runnable{

    //第一个端口
    private Integer port1 = 8111;
    //第二个端口
    private Integer port2 = 8222;
    //第一个服务器通道 服务A
    ...
    public void accept(SelectionKey key) throws IOException {
        ...
        if (server.equals(serversocket1)) {
            ...
            System.out.println("当前server_1 被连接客户端数量："+count1.addAndGet(1));
        } else {
            ...
            System.out.println("当前server_2 被连接客户端数量："+count2.addAndGet(1));
        }
    }
    ...
    public void read(SelectionKey key) throws IOException {
        ...
        if(StringUtils.isEmpty(input)){
            ...
            if (channel.equals(this.clientchannel1)) {
                System.out.println("欢迎您使用服务A， 您的输入为："+input);
            } else {
                System.out.println("欢迎您使用服务B,  您的输入为："+input);
            }
        }
    ...
    public void run() {
        while (true) {
            try {
                System.out.println("running ... "+"                   当前线程------->"+Thread.currentThread().getName());
                //选择一组键，其相应的通道已为 I/O 操作准备就绪。  select是会被阻塞的
                this.selector.select();
                System.out.println("select 阻塞完成 ... ");
                //返回此选择器的已选择键集
                ...
                while (selectorKeys.hasNext()) {
                    ...
                    System.out.println("running ... "+key.toString());
                    //然后将它从返回键队列中删除
                    selectorKeys.remove();
                    if (!key.isValid()) { // 选择键无效
                        System.out.println("key无效 "+key.toString());
                        continue;
                    }
                    if (key.isAcceptable()) {
                        System.out.println("key Acceptable "+key.toString()+"     当前线程------->"+Thread.currentThread().getName());
                        //如果遇到请求那么就响应
                        this.accept(key);
                    } else if (key.isReadable()) {
                        System.out.println("key Readable "+key.toString()+"       当前线程------->"+Thread.currentThread().getName());
                        //读取客户端的数据
                        this.read(key);
                    }
                }
