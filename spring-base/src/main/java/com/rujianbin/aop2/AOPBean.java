...
public class AOPBean {
	...
	@Pointcut("execution(* com.rujianbin.aop2.YellowHuman.*(..))")
	public void aspect(){	}
	...
	@AfterReturning(returning="returning",pointcut="execution(* com.rujianbin.aop2.YellowHuman.talk(..))")
	public void afterMethodUseReturn(String returning){
		System.out.println("@AfterReturning执行方法【后】切面逻辑，   目标方法返回值="+returning);
	}
	
	
	@AfterThrowing(throwing="ex",pointcut="execution(* com.rujianbin.aop2.YellowHuman.think(..))")
	public void afterMethodThrowing(Throwable ex){
		System.out.println("@AfterThrowing执行方法【后】切面逻辑，   目标方法抛异常="+ex.getMessage());
	}
	
	
	@Around("execution(* com.rujianbin.aop2.YellowHuman.talk(..))")
	...
	@AfterReturning(returning="returning",pointcut="execution(* com.rujianbin.aop2.YellowHuman.getDate(..)) && args(d,f,..)")
